      ...
      <div className="relative w-full max-w-xl rounded-2xl border border-white/15 bg-[#0b0f14] p-5">
        ...
        {step === "form" && (
          <form onSubmit={submitEmailOtp} className="grid gap-3 mt-4">
            ...
            <label className="text-sm flex items-start gap-2"><input type="checkbox" checked={form.consent_marketing} onChange={(e) => setForm((s) => ({ ...s, consent_marketing: e.target.checked }))} /> Consent to send marketing announcements and opportunities on email and WhatsApp</label>
            <label className="text-sm flex items-start gap-2"><input type="checkbox" checked={form.consent_newsletter} onChange={(e) => setForm((s) => ({ ...s, consent_newsletter: e.target.checked }))} /> Consent to send newsletter through email</label>
