angular.module('myAppWIPCtrl', []).controller('wipCtrl', ['$scope', 'homeContent', function($scope, homeContent){
...
Sun = function(){
	...
	const sunTexture = new THREE.TextureLoader().load('/assets/images/website/defaultProfilePic.png');
...
MissileGroundLauncher = function() {
    this.mesh = new THREE.Object3D();

    // Car Body
    var geomCar = new THREE.BoxGeometry(20, 10, 10);
    var matCar = new THREE.MeshPhongMaterial({color: 0x000000}); // Black color
    var car = new THREE.Mesh(geomCar, matCar);
    car.castShadow = true;
...
Missile = function() {
    ...
    // Missile Body (Cube)
    var geomMissile = new THREE.BoxGeometry(4, 4, 15); // Width, height, and depth
    var matMissile = new THREE.MeshPhongMaterial({color: 0x0000ff}); // Blue color
    var missile = new THREE.Mesh(geomMissile, matMissile);
    missile.castShadow = true;
    missile.receiveShadow = true;
    this.mesh.add(missile);

    // Red Triangle at Bottom
    var geomTriangle = new THREE.ConeGeometry(2, 10, 3); // Base radius, height, and number of sides
    var matTriangle = new THREE.MeshPhongMaterial({color: 0xff0000}); // Red color
    var triangle = new THREE.Mesh(geomTriangle, matTriangle);
    triangle.position.set(0, -2, -12.5); // Adjust the position to align with the bottom face of the cube
    triangle.rotation.set(-90, 0, 0); // Ensure the triangle is aligned correctly along the missile's Z-axis
...
Forest = function(){
	...
	var stepAngle = Math.PI*2 / this.nlaunchers;


	for(var i=0; i<this.nlaunchers; i++){	

		var f = new MissileGroundLauncher();
		var a = stepAngle*i;
		launchers.push(f);
...
function updatePlane() {
    ...
    var deltaY = (targetY - airplane.mesh.position.y) * 0.1;
    var deltaX = (targetX - airplane.mesh.position.x) * 0.1;

    // Apply boost if active
    if (airplane.boostActive) {
        deltaY *= 2; // Double speed during boost
        deltaX *= 2;
    }

    // Update position
    airplane.mesh.position.y += deltaY;
    airplane.mesh.position.x += deltaX;

    // Rotate the plane proportionally to the movement
    airplane.mesh.rotation.z = deltaY * 0.0128;
    airplane.mesh.rotation.x = -deltaY * 0.0064; // Invert for a smoother tilt
    airplane.mesh.rotation.y = -deltaX * 0.0064;

    // Propeller animation
    airplane.propeller.rotation.x += airplane.boostActive ? 1.2 : 0.3; // Faster propeller during boost
}

function launchMissileFromAirPlane(source) {
    var missile = new Missile();
    missile.mesh.position.set(source.mesh.position.x, source.mesh.position.y - 20, source.mesh.position.z); // Start missile from the ground
	
	
    // Rotate missile 90 degrees to point forward
    missile.mesh.rotation.y = -Math.PI / 2;

	scene.add(missile.mesh);

    // Animate fall
	var startX = missile.mesh.position.x;
    var startY = missile.mesh.position.y;
	var endX = missile.mesh.position.x + 30;
    var endY = missile.mesh.position.y - 15; // Fall 10 units
    var duration = 500; // Duration of fall animation in milliseconds
    ...
    function animateFall() {
		...
		var t = Math.min(elapsedTime / duration, 1); // Normalize t between 0 and 1
		missile.mesh.position.y = startY + (endY - startY) * t;
		missile.mesh.position.x = startX + (endX - startX) * t;
	
		if (t < 1) {
			requestAnimationFrame(animateFall);
		} else {
			missile.moveForward(10); // After fall, start moving forward
		}
	}
...
function launchMissileToAirplane(target, source) {

    var missile = new Missile();
    missile.mesh.position.set(source.mesh.position.x - 400, source.mesh.position.y - 90, source.mesh.position.z); // Start missile from the ground
    // Rotate missile 90 degrees to point forward
    missile.mesh.rotation.x = Math.PI / 2;
    
    scene.add(missile.mesh);

    // Initial Y-direction movement
    var startY = missile.mesh.position.y;
    var endY = target.mesh.position.y + 20; // Move 20 units upwards initially
    var durationY = 4600; // Duration of Y movement in milliseconds

    var startTime = Date.now();

    // Define initial start point
    const startPosition = new THREE.Vector3(
        source.mesh.position.x + 400,
        source.mesh.position.y - 90,
        source.mesh.position.z
    );

    let midPoint = null;
    let typeOfMissile = 3;
    // Midpoint: Vertical lift, close to the source
    switch(typeOfMissile)
    {
        case 1: {
            midPoint = new THREE.Vector3(
                startPosition.x, // Keep horizontally aligned with the start
                source.mesh.position.y + 100, // Move upward along Y-axis
                startPosition.z // Keep horizontally aligned with the start
            );
            break;
        }
        case 2: {
            midPoint = new THREE.Vector3(
                ...
                source.mesh.position.y + 500, // Raise mid-point for the curve
    ...
    function animateYMovement() {
        var currentTime = Date.now();
        var elapsedTime = currentTime - startTime;
        var t = Math.min(elapsedTime / durationY, 1); // Normalize t between 0 and 1

        var position = curve.getPoint(t);
        missile.mesh.position.set(position.x, position.y, position.z);

        // Correct rotation towards target
        var forwardDirection = new THREE.Vector3().subVectors(target.mesh.position, missile.mesh.position).normalize();
        missile.mesh.lookAt(missile.mesh.position.clone().add(forwardDirection));

        if (t < 1) {
            requestAnimationFrame(animateYMovement);
        }
    }
...
function findUpcomingLaunchers(airplane, launchers) {
    let planeDirection = new THREE.Vector2(
        Math.sin(airplane.mesh.rotation.y), // x direction
        Math.cos(airplane.mesh.rotation.y)  // z direction
    );

    for (let i = 0; i < launchers.length; i++) {
        let launcher = launchers[i];
        if (launcher && launcher.mesh) {
            let launcherPosition = new THREE.Vector2(
                launcher.mesh.position.x - airplane.mesh.position.x,
                launcher.mesh.position.z - airplane.mesh.position.z
            ).normalize();

            // Dot product to determine alignment with plane's forward direction
            let dotProduct = planeDirection.dot(launcherPosition);
            if (dotProduct > 0.95) { // Threshold close to 1 for direct alignment
                return launcher; // Return the next aligned launcher
            }
        }
    }

    console.log("nope");
    return null; // No upcoming launcher found
}

// Missile.prototype.moveForward = function() {
//     var moveMissile = function() {
//         // Move the missile forward along the x-axis
//         this.mesh.position.x += 10; // Adjust speed if necessary

//         // Boundary check to prevent exceeding maximum range
//         if (this.mesh.position.x > 1000) { // Adjust boundary as needed for scene dimensions
//             this.mesh.parent.remove(this.mesh); // Remove missile after reaching maximum range
//             return;
//         }

//         // Continue moving forward
//         requestAnimationFrame(moveMissile.bind(this));
//     }.bind(this);

//     moveMissile();
// };

Missile.prototype.moveForward = function(speed) {
    var moveMissile = function() {
        // Get the forward direction from the missile's rotation
        var forwardDirection = new THREE.Vector3(0, 0, -1); // Assuming the missile is initially looking down the -Z axis
        forwardDirection.applyQuaternion(this.mesh.quaternion); // Apply rotation to get actual forward vector

        // Multiply the direction vector by speed
        this.mesh.position.add(forwardDirection.clone().multiplyScalar(speed));

        // Boundary check to prevent exceeding maximum range
        if (this.mesh.position.x > 1000 || this.mesh.position.y > 1000 || this.mesh.position.z > 1000) { 
            this.mesh.parent.remove(this.mesh); // Remove missile after reaching maximum range
			return;
		}
...
function launchMissileFromTargetToSource(target, source, typeOfMissile) {
    // Missile creation
    var missile = new Missile();
    missile.mesh.position.set(source.mesh.position.x + 400, source.mesh.position.y - 90, source.mesh.position.z); // Start missile from the ground
    scene.add(missile.mesh);
    ...
    let t = 0;
    const speed = 0.001; // Adjust speed as necessary

    let homing = true; // Flag to control the homing behavior
    let homingDuration = 3; // Duration for homing in seconds
	
    function animateMissile() {
        if (homing && t <= homingDuration) { // Homing for the set duration
            ...
            // Recalculate the curve with updated target position
			const curve = new THREE.CatmullRomCurve3([startPosition, midPoint, targetPosition]);
            ...
            if (t + speed <= homingDuration) {
                const nextPosition = curve.getPointAt(t + speed);
                const direction = nextPosition.clone().sub(position).normalize();

                // Create a quaternion for rotation adjustment
                const quaternion = new THREE.Quaternion();
                const up = new THREE.Vector3(0, 1, 0); // Define the missile's forward axis (+Y)
                quaternion.setFromUnitVectors(up, direction);

                // Apply the quaternion to the missile's rotation
                missile.mesh.setRotationFromQuaternion(quaternion);
            }

            t += speed; // Increment t
            requestAnimationFrame(animateMissile);
        } else if (t > homingDuration) { 
            // After homing phase, move along a fixed axis
            const direction = new THREE.Vector3(0, -1, 0); // Example direction, modify as needed
            missile.mesh.position.add(direction.clone().multiplyScalar(speed));
            requestAnimationFrame(animateMissile);
        }
    }
...
function init(event) {
	...
	// Click event listener to launch missile
	document.addEventListener('click', function(event) {
		...
		launchMissileFromAirPlane(airplane);
		// Usage
		
		let thresholdDistance = 700; // Define a threshold distance
		// let source = findUpcomingLaunchers(airplane, launchers, thresholdDistance);
		// launchMissileFromTargetToSource(airplane, airplane, 1);
		launchMissileToAirplane(airplane, airplane);
    });
