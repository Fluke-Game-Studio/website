...
export function useApplicationController(roleTitle: string | null) {
    ...
    useEffect(() => {
        if (!roleTitle) {
            setLoading(false);
            setError('Select a role before applying.');
            return;
        }
        let cancelled = false;
    ...
    function validateChapter(chapter: any) {
        ...
        for (const field of chapter.fields) {
            if (field.required) {
                ...
                const isEmptyString = typeof val === 'string' && String(val).trim() === '';
                if (val === undefined || val === null || isEmptyArray || isEmptyString || val === false) {
                    return false;
                }
            }
        }
